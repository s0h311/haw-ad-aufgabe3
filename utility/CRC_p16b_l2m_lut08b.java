...
class CRC_p16b_l2m_lut08b {                                                     // package scope on purpose resp. NOT for public usage
  ...
  static String getDecodedVersion() {
  ...
  CRC_p16b_l2m_lut08b(final int primePolynomial) {                           // package scope on purpose resp. NOT for public usage
    ...
    if (0 == primePolynomial)
      throw new IllegalArgumentException("polynomial MUST NOT be 0 - this is definitly false for a prime polynomial");
    if (1 != (0b1 & primePolynomial))
      throw new IllegalArgumentException("LSB MUST NOT be 0 - this is definitly false for a prime polynomial");

    final int[] tmp = mirror(primePolynomial);
    if (17 != tmp[1])
      throw new IllegalArgumentException(String.format("17 bit are expected for 16 bit signature and NOT %d bit", tmp[1]));
    ...
    if (expectedChkSgn != chkSign || expectedStepCnt != chkStepCnt)
      throw new IllegalArgumentException(String.format("0x%x is NO prime polynomial", primePolynomial));
